...
/// The generation type currently active (null when idle).
 ...
 Duration? get totalRepeatDuration;/// Countdown of total remaining time across all cycles (null for infinite).
...
@override
int get hashCode => Object.hash(runtimeType,activeType,status,ambienceId,formulaId,const DeepCollectionEquality().hash(sequence),currentCycle,currentStepIndex,totalCycleDuration,firstCycleDuration,remainingInCycle,isFirstCycle,errorMessage,gracefulStopRequested,repeatCount,totalRepeatDuration,totalRemaining);
...
@useResult
$Res call({
 GenerationType? activeType, HarmonizerStatus status, String? ambienceId, String? formulaId, List<HarmonicPacketDto> sequence, int currentCycle, int currentStepIndex, Duration totalCycleDuration, Duration firstCycleDuration, Duration remainingInCycle, bool isFirstCycle, String? errorMessage, bool gracefulStopRequested, int? repeatCount, Duration? totalRepeatDuration, Duration? totalRemaining
});
...
as Duration?,totalRemaining: freezed == totalRemaining ? _self.totalRemaining : totalRemaining // ignore: cast_nullable_to_non_nullable
as Duration?,
  ));
}
...
@optionalTypeArgs TResult when<TResult extends Object?>(TResult Function( GenerationType? activeType,  HarmonizerStatus status,  String? ambienceId,  String? formulaId,  List<HarmonicPacketDto> sequence,  int currentCycle,  int currentStepIndex,  Duration totalCycleDuration,  Duration firstCycleDuration,  Duration remainingInCycle,  bool isFirstCycle,  String? errorMessage,  bool gracefulStopRequested,  int? repeatCount,  Duration? totalRepeatDuration,  Duration? totalRemaining)  $default,) {final _that = this;
switch (_that) {
case _HarmonizerState():
return $default(_that.activeType,_that.status,_that.ambienceId,_that.formulaId,_that.sequence,_that.currentCycle,_that.currentStepIndex,_that.totalCycleDuration,_that.firstCycleDuration,_that.remainingInCycle,_that.isFirstCycle,_that.errorMessage,_that.gracefulStopRequested,_that.repeatCount,_that.totalRepeatDuration,_that.totalRemaining);case _:
  throw StateError('Unexpected subclass');
...
class _HarmonizerState implements HarmonizerState {
  const _HarmonizerState({this.activeType, this.status = HarmonizerStatus.idle, this.ambienceId, this.formulaId, final  List<HarmonicPacketDto> sequence = const [], this.currentCycle = 0, this.currentStepIndex = 0, this.totalCycleDuration = Duration.zero, this.firstCycleDuration = Duration.zero, this.remainingInCycle = Duration.zero, this.isFirstCycle = true, this.errorMessage, this.gracefulStopRequested = false, this.repeatCount, this.totalRepeatDuration, this.totalRemaining}): _sequence = sequence;
...
@override
int get hashCode => Object.hash(runtimeType,activeType,status,ambienceId,formulaId,const DeepCollectionEquality().hash(_sequence),currentCycle,currentStepIndex,totalCycleDuration,firstCycleDuration,remainingInCycle,isFirstCycle,errorMessage,gracefulStopRequested,repeatCount,totalRepeatDuration,totalRemaining);
